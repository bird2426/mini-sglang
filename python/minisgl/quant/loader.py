...
def _parse_awq_config(
    model_path: str, state_dict: Dict[str, torch.Tensor]
...
def shard_quantized_state_dict(
    ...
    for key, value in state_dict.items():
        ...
        if any(
            pattern in base_key
            for pattern in (".q_proj", ".k_proj", ".v_proj", ".gate_proj", ".up_proj")
        ):
...
def create_quant_weight_mapping(
    state_dict: Dict[str, torch.Tensor],
    quant_config,
) -> Dict[str, torch.Tensor]:
    """Create mapping from quantized weight keys to layer attribute keys.

    This function analyzes the quantized state_dict and creates a mapping
    that connects quantized weight keys (qweight, scales, qzeros) to the
    corresponding layer attributes.

    Args:
        state_dict: The quantized state dictionary.
        quant_config: The quantization configuration.

    Returns:
        Mapping from layer attribute keys to quantized tensor values.
    """
    ...
    for key, value in state_dict.items():
        ...
        if base_key not in layer_weights:
            layer_weights[base_key] = {}
        layer_weights[base_key][suffix[1:]] = value  # Remove the dot

    # Create layer attribute mapping
    for base_key, weights in layer_weights.items():
        if "qweight" in weights and "scales" in weights:
            mapping[f"{base_key}.qweight"] = weights["qweight"]
            mapping[f"{base_key}.scales"] = weights["scales"]
            if "qzeros" in weights:
                mapping[f"{base_key}.qzeros"] = weights["qzeros"]
